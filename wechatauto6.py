...
print(df)
window_name = pd.read_csv('../window_name.txt', encoding='utf-8').columns[0]
print(window_name)
my_name = pd.read_csv('../my_name.txt', encoding='utf-8').columns[0]
print(my_name)

conversations = hw.GetChildren()  # GetChildren()方法，获取会话列表中的所有子控件。
for conversation in conversations:
    contact_name = conversation.Name
    if contact_name == window_name:
        # 死循环接收消息
        while True:
